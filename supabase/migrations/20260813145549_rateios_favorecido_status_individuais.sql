-- Permite definir favorecido e status de pagamento individualmente em cada rateio.
-- Os campos centrais continuam servindo como fallback para clientes antigos.
...
begin
  ...
  loop
    ...
    v_favorecido := coalesce(
      nullif(btrim(coalesce(v_rateio->>'favorecido', '')), ''),
      nullif(btrim(coalesce(p_dados->>'favorecido', '')), '')
    );
